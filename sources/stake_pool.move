module stake_pool_addr::stake_pool {
        ...
        move_to(
            ...
            UserStakeController {
                extend_ref: object::generate_extend_ref(
                    user_stake_controller_constructor_ref
                )
            }
        );
    ...
    public entry fun update_reward_creator(
        sender: &signer, new_reward_creator: address
    ) acquires Config {
        let sender_addr = signer::address_of(sender);
        let config = borrow_global_mut<Config>(@stake_pool_addr);
    ...
    public entry fun create_reward_schedule(
        ...
        assert!(
            config.reward_creator == sender_addr,
            ERR_ONLY_REWARD_CREATOR_CAN_ADD_REWARD
        );

        let total_reward_amount = rps * duration_seconds;
        let stake_pool_mut = borrow_global_mut<StakePool>(@stake_pool_addr);
        assert!(
            option::is_none(&stake_pool_mut.reward_schedule),
            ERR_REWARD_SCHEDULE_ALREADY_EXISTS
        );
        assert!(
            primary_fungible_store::balance(
                sender_addr, stake_pool_mut.fa_metadata_object
            ) >= total_reward_amount,
            ERR_NOT_ENOUGH_BALANCE_TO_ADD_REWARD
        );

        stake_pool_mut.reward_schedule = option::some(
            RewardSchedule {
    ...
    public entry fun claim_reward(
        ...
        let claimable_reward =
            get_claimable_reward_helper(stake_pool, sender_addr, current_ts);
        if (claimable_reward == 0) { return };
    ...
    public entry fun stake(
        ...
        assert!(
            primary_fungible_store::balance(sender_addr, stake_pool.fa_metadata_object)
                >= amount,
            ERR_NOT_ENOUGH_BALANCE_TO_STAKE
        );
        let (stake_store, is_new_stake_store) =
            get_or_create_user_stake_store(stake_pool.fa_metadata_object, sender_addr);
        fungible_asset::transfer(
            sender,
            primary_fungible_store::primary_store(
                sender_addr, stake_pool.fa_metadata_object
            ),
            stake_store,
            amount
        );
        ...
        let user_stake_mut =
            borrow_global_mut<UserStake>(get_user_stake_object_address(sender_addr));
    ...
    public entry fun unstake(
        sender: &signer, amount: Option<u64>
    ) acquires StakePool, FungibleStoreController, UserStake, UserStakeController {
        let current_ts = timestamp::now_seconds();
        let sender_addr = signer::address_of(sender);
        let stake_pool = borrow_global<StakePool>(@stake_pool_addr);
        ...
        let user_stake =
            borrow_global<UserStake>(get_user_stake_object_address(sender_addr));
        let updated_amount =
    ...
    #[view]
    /// Get stake pool data
    public fun get_stake_pool_data(): (Object<Metadata>, Object<FungibleStore>, u64, u64) acquires StakePool {
        let stake_pool = borrow_global<StakePool>(@stake_pool_addr);
        (
    ...
    public fun get_reward_schedule(): (FixedPoint64, u64, u64, u64, u64) acquires StakePool {
        let stake_pool = borrow_global<StakePool>(@stake_pool_addr);
    ...
    public fun get_reward_released_so_far(): u64 acquires StakePool {
        ...
        (math64::min(current_ts, reward_schedule.end_ts) - reward_schedule.start_ts)
            * reward_schedule.rps
    }
    ...
    #[view]
    /// Get user stake data
    public fun get_user_stake_data(
        user_addr: address
    ): (u64, u64, FixedPoint64) acquires UserStake, UserStakeController {
        let user_stake =
            borrow_global<UserStake>(get_user_stake_object_address(user_addr));
        (user_stake.amount, user_stake.last_claim_ts, user_stake.index)
    }

    #[view]
    /// Get claimable reward
    public fun get_claimable_reward(
        user_addr: address
    ): u64 acquires StakePool, UserStake, UserStakeController {
        let stake_pool = borrow_global<StakePool>(@stake_pool_addr);
    ...
    fun abort_if_reward_schedule_has_finished(current_ts: u64) acquires StakePool {
        ...
        if (option::is_some(&stake_pool.reward_schedule)) {
            let reward_schedule = option::borrow(&stake_pool.reward_schedule);
            assert!(
                current_ts < reward_schedule.end_ts,
                ERR_CANNOT_STAKE_AFTER_REWARD_SCHEDULE_HAS_FINISHED
            );
        }
    }
    ...
    fun generate_fungible_store_signer(): signer acquires FungibleStoreController {
        object::generate_signer_for_extending(
            &borrow_global<FungibleStoreController>(@stake_pool_addr).extend_ref
        )
    }
    ...
    fun generate_user_stake_object_signer(): signer acquires UserStakeController {
        object::generate_signer_for_extending(
            &borrow_global<UserStakeController>(@stake_pool_addr).extend_ref
        )
    }
    ...
    fun construct_user_stake_object_seed(user_addr: address): vector<u8> {
        bcs::to_bytes(
            &string_utils::format2(&b"{}_staker_{}", @stake_pool_addr, user_addr)
        )
    }
    ...
    fun calculate_new_reward_index(
        reward_schedule: &Option<RewardSchedule>, current_ts: u64, total_stake: u64
    ): FixedPoint64 {
        if (option::is_none(reward_schedule) || total_stake == 0) {
            ...
            fixed_point64::add(
                ...
                fixed_point64::create_from_rational(
                    (
                        (
                            math64::min(current_ts, reward_schedule.end_ts)
                                - math64::min(
                                    reward_schedule.last_update_ts,
                                    reward_schedule.end_ts
                                )
    ...
    fun get_claimable_reward_helper(
        stake_pool: &StakePool, user_addr: address, current_ts: u64
        ...
        let updated_reward_index =
            calculate_new_reward_index(
    ...
    fun get_or_create_user_stake_store(
        fa_metadata_object: Object<Metadata>, user_addr: address
        ...
        if (object::object_exists<UserStake>(user_stake_object_addr)) {
            ...
            let stake_store_object_constructor_ref =
    ...
    fun transfer_reward_to_claimer(
        claimable_reward: u64, user_addr: address, stake_pool: &StakePool
    ...
    fun update_reward_index_and_claim_ts(
        user_addr: address, current_ts: u64
    ) acquires StakePool, UserStake, UserStakeController {
        let stake_pool_mut = borrow_global_mut<StakePool>(@stake_pool_addr);
        let user_stake_mut =
            borrow_global_mut<UserStake>(get_user_stake_object_address(user_addr));
        if (option::is_none(&stake_pool_mut.reward_schedule)) { return };
        ...
        let reward_schedule_mut = option::borrow_mut(
            &mut stake_pool_mut.reward_schedule
        );
    ...
    fun create_new_user_stake_object(
        user_addr: address, stake_store: Object<FungibleStore>, current_ts: u64
    ) acquires StakePool, UserStakeController {
        let stake_pool_mut = borrow_global_mut<StakePool>(@stake_pool_addr);
        stake_pool_mut.unique_stakers = stake_pool_mut.unique_stakers + 1;
        let user_stake_object_constructor_ref =
            &object::create_named_object(
                &generate_user_stake_object_signer(),
                construct_user_stake_object_seed(user_addr)
            );
        move_to(
            &object::generate_signer(user_stake_object_constructor_ref),
            UserStake {
                stake_store,
