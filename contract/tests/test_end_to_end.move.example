...
module stake_pool_addr::test_end_to_end {
    ...
    fun test_happy_path(
        ...
        let fa_metadata_object =
            object::object_from_constructor_ref(fa_obj_constructor_ref);
        ...
        */
        ...
        let (user_staked_amount, userlast_claim_ts, user_index) =
            stake_pool::get_user_stake_data(staker1_addr);
        let claimable_reward = stake_pool::get_claimable_reward(staker1_addr);
        debug::print(
            &string_utils::format1(&b"user_staked_amount: {}", user_staked_amount)
        );
        debug::print(
            &string_utils::format1(&b"userlast_claim_ts: {}", userlast_claim_ts)
        );
        ...
        */
        ...
        debug::print(
            &string_utils::format1(&b"reward_schedule_index: {}", reward_schedule_index)
        );
        debug::print(
            &string_utils::format1(&b"reward_schedule_rps: {}", reward_schedule_rps)
        );
        debug::print(
            &string_utils::format1(
                &b"reward_schedule_last_update_ts: {}", reward_schedule_last_update_ts
            )
        );
        debug::print(
            &string_utils::format1(
                &b"reward_schedule_start_ts: {}", reward_schedule_start_ts
            )
        );
        debug::print(
            &string_utils::format1(
                &b"reward_schedule_end_ts: {}", reward_schedule_end_ts
            )
        );

        let staker1_reward_balance =
            primary_fungible_store::balance(staker1_addr, fa_metadata_object);
        assert!(staker1_reward_balance == 25298, staker1_reward_balance); // not 25300 because of the rounding
        let staker2_reward_balance =
            primary_fungible_store::balance(staker2_addr, fa_metadata_object);
